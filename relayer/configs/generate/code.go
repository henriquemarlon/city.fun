...
var funcMap = template.FuncMap{
	"toFieldName": func(env string) string {
		...
		if len(words) > 1 && words[0] == "RELAYER" {
			words = words[1:]
		}
	...
	"toConstName": func(s string) string {
		return strings.TrimPrefix(s, "RELAYER_")
	},
	...
	"toGoFunc": func(goType string) string {
		// Handle slice types like []URL, []string
		if strings.HasPrefix(goType, "[]") {
			elemType := goType[2:]
			return "toSlice" + strings.ToUpper(elemType[:1]) + elemType[1:]
		}
		return "to" + strings.ToUpper(goType[:1]) + goType[1:]
	},
	"notDefinedFunc": func(goType string) string {
		// Handle slice types like []URL, []string
		if strings.HasPrefix(goType, "[]") {
			elemType := goType[2:]
			return "notDefinedSlice" + strings.ToUpper(elemType[:1]) + elemType[1:]
		}
		return "notDefined" + strings.ToUpper(goType[:1]) + goType[1:]
	},
...
func Get{{ toFieldName .Name }}() ({{ .GoType }}, error) {
	...
	if s == "" {
		...
		if err != nil {
			return {{ notDefinedFunc .GoType }}(), fmt.Errorf("failed to parse %s: %w", {{ toConstName .Name }}_FILE, err)
		}
	...
	return {{ notDefinedFunc .GoType }}(), fmt.Errorf("%s: %w", {{ toConstName .Name }}, ErrNotDefined)
}
{{ end }}
`
